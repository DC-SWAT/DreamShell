...
typedef struct _mxml_fdbuf_s		/**** File descriptor buffer ****/
{
  ...
  unsigned char	*current,		/* Current position in buffer */
		*end,			/* End of buffer */
		buffer[8192] __attribute__((aligned(32)));		/* Character buffer */
} _mxml_fdbuf_t;
...
//  char	buffer[8192];			/* Temporary buffer */
  ...
  */
  ...
  bytes = mxmlSaveString(node, s, bs, cb);
    ...
    */
	...
	else if (ch == '&')
	{
	  ...
	  goto error;
	         ...
	         ((bufptr - buffer) == 3 && !strncasecmp(buffer, "!--", 3)) ||
