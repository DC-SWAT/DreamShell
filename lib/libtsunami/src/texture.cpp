...
extern "C"
{
	...
	Texture* TSU_TextureCreateFromFile(const char *texture_path, bool use_alpha, bool yflip = false)
	{
		if (texture_path != NULL) {
			return new Texture(texture_path, use_alpha, yflip);
		}
		else {
			return NULL;
		}
	...
	{
		if (texture_ptr != NULL) {
			return texture_ptr->loadFromFile(texture_path, use_alpha, yflip);
		}
		else {
			return false;
		}
