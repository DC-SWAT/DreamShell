/*
  ...
  Copyright (C) 2014 SWAT <swat@211.ru>
...
typedef struct {
	...
	SDL_RWops *ctx;
	Uint8 buffer[INPUT_BUFFER_SIZE] __attribute__((aligned(32)));
} my_source_mgr;
...
static boolean fill_input_buffer (j_decompress_ptr cinfo)
{
	...
	if(fd > -1) {
		nbytes = read(fd, src->buffer, INPUT_BUFFER_SIZE);
	} else {
		SDL_RWread(src->ctx, src->buffer, INPUT_BUFFER_SIZE, 1);
	}
#else
