	...
	for (m=0; m<4; ++m) { // 4 sboxes
		for (k=0, aux=0; k<6; ++k)
			if (sboxes[m].inputs[k]!=255)
				aux |= CBIT(input, sboxes[m].inputs[k]) << k;

		aux = sboxes[m].table[(aux^subkeys)&0x3f];

		for (k=0; k<2; ++k)
			result |= CBIT(aux,k) << sboxes[m].outputs[k];
...
static u16 block_decrypt(uint32_t game_key, uint16_t sequence_key, uint16_t counter, uint16_t data)
{
	...
	for (j = 0; j < FN1GK; ++j) {
		if (CBIT(game_key, fn1_game_key_scheduling[j][0]) != 0) {
	...
	for (j = 0; j < FN2GK; ++j) {
		if (CBIT(game_key, fn2_game_key_scheduling[j][0]) != 0) {
	...
	for (j = 0; j < 20; ++j) {
		if (CBIT(sequence_key, fn1_sequence_key_scheduling[j][0]) != 0) {
...
static u16 m_read(uint32_t addr)
{
	...
	return (u16)(s_rom[b + 1] | (s_rom[b] << 8));
}
...
void naomi_5881_decrypt(void *dst, uint32_t size, uint32_t word_addr, uint16_t subkey, uint32_t key)
{
	...
	cryptoReset();
	cyptoSetKey(key);
	cyptoSetHighAddr((u16)(word_addr >> 16));
	cyptoSetLowAddr((u16)word_addr);
	cyptoSetSubkey(subkey);
	for(i = 0; i < n; i++) {
		u16 w = cryptoDecrypt();
		out[i * 2] = (u8)w;
		out[i * 2 + 1] = (u8)(w >> 8);
