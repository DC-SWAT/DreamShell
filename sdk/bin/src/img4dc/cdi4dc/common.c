...
int write_data_track(FILE* cdi, FILE* iso) {
	...
	unsigned long const iso_size = fsize(iso);
...
void write_cdi_head_end(FILE* cdi, char* volumename, int total_cdi_space_used, long cdi_end_image_tracks) {

	int i;
	unsigned char volumename_length;
	unsigned int cdi_header_pos;
