...
unsigned long fsize(FILE *stream) {
	...
	unsigned long const curpos = ftell(stream); /* garder la position courante */
...
int check_iso_is_bootable(FILE* iso) {
	...
	unsigned long const curpos = ftell(iso); /* garder la position courante */
	fseek(iso, 0L, SEEK_SET);

	{	size_t const read = fread(buf, sizeof(buf), 1, iso);
		if (read != sizeof(buf)) abort();
	}
	buf[sizeof(buf) - 1] = '\0';
...
//#endif
	...
	{	size_t const read = fread(&result, sizeof(result), 1, iso);
		if (read != sizeof(result)) abort();
	}
	// result[i] = '\0';
...
int get_iso_msinfo_value(FILE* iso) {
	...
	{	size_t const read = fread(&lba, sizeof(lba), 1, iso); // récuperer la valeur
		if (read != sizeof(lba)) abort();
	}
...
// note : an fseek() would achieve the same outcome
...
void write_array_block(FILE* cdi, int array_size, const int array_entries, const unsigned int values_array[][2]) {
	unsigned char* const buf = malloc(array_size);
	if (buf==NULL) abort();
...
void get_full_filename(char* in, char* result) {
	...
	long const retval = GetFullPathName(in, BUFSIZE, buffer, lpPart);
...
void textcolor(int attr, int fg, int bg) {
	...
	// Note : better be sure that only <=13 bytes are written ...
