...
/***************************************************************************
 * $Log: ini.cpp,v $
 * Revision 1.39  2009/08/24 21:21:06  s_a_white
 * Remove the need for a temp file when the ini file is opened read only
 *
...
int __ini_process (ini_t *ini, FILE *file, const char *comment)
{
    ...
    do
    {
        ...
        while (count--)
        {
            ...
            // Backup file nolonger used in read only mode
            if (ini->mode != INI_READ)
                fputc (ch, ini->ftmp);
