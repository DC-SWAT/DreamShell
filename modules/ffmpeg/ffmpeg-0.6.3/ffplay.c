...
static int video_open(VideoState *is){
    ...
    screen = GetScreen(); //SDL_SetVideoMode(w, h, 24, flags);
#endif
    if (!screen) {
...
static int refresh_thread(void *opaque)
{
    ...
    SDL_PushEvent(&event);
        ...
        thd_sleep(is->audio_st && is->show_audio ? rdftspeed : 5); //FIXME ideally we should wait the correct time but SDLs event passing is so slow it would be silly
    }
    return 0;
}
